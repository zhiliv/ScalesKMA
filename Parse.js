...
function DefineError(err, FuncName) {
	if (err)
		throw console.log(
			clc.red(moment().format('LLL') + ' Произошла ошибка:' + err + '\n' + 'в функции: ' + FuncName)
		);
}
...
function GetDate(FileName) {
	...
	var ParamDate = new Date(DataYear, DataMonth - 1, DataDay, DataHour, DataMinute, DataSeconds); //формирование значени едаты для записи в БД
	return ParamDate;
}
...
function GetDataFile(DirName, TypeScales, Scales, callback) {
	...
	ListFile.forEach((FileName, index) => {
		...
		if (ExplansioFile == '.xml') {
			...
			parser.parseString(contant, (err, result) => {
				//  парсинг данных
...
exports.FillScales = ScalesAdress => {
	...
	DB.GetBruttoAdress([[ScalesAdress]], BruttoAdress => {
